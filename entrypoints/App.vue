      ...
      <div class="menu-flex">
        ...
        <div class="menu-body">
          <div class="menu-body-item" v-show="activeIndex == 0">
            ...
            <!-- 新增图片预览增强功能开关 -->
            <MenuImagePreview :sort="38" v-model="settingData.checked47" />
          </div>
...
import MenuImagePreview from "./components/BasicSettings/MenuImagePreview.vue";
  ...
  methods: {
    ...
    setStorage(data) {
      ...
      if (data) {
        ...
        localStorage.setItem("linxudoscriptssettingDMI", JSON.stringify(data));
      }
      // 无需其他操作，在created中已经处理了本地存储的初始化
    },
    // 打开设置
    setting() {
    ...
    initFeatures() {
      ...
      // 初始化图片预览增强功能
      if (this.settingData.checked47) {
        this.initImageViewer();
      }
      
      // 监听设置变化，如果图片预览增强功能开关变化，动态加载或卸载
      this.$watch('settingData.checked47', newVal => {
        newVal ? this.initImageViewer() : this.destroyImageViewer();
      });
    },
    
    // 初始化图片查看器
    initImageViewer() {
      import('./components/ImageViewer/ImageViewer.js')
        .then(module => module.initializeImageViewer());
    },
    
    // 销毁图片查看器
    destroyImageViewer() {
      import('./components/ImageViewer/ImageViewer.js')
        .then(module => module.destroyImageViewer());
    },
  },
  created() {
